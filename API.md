...
AI News Assistant Bot предоставляет RESTful API через Telegram Bot API для управления новостными дайджестами, модерации контента и взаимодействия с экспертами.
...
#### `expert_comment_{expert_id}_{news_id}`
**Описание**: Запрос комментария от эксперта
**Доступ**: Эксперты
**Процесс**:
1. Отправка новости эксперту
2. Ожидание комментария
3. Сохранение комментария
4. Интеграция в дайджест

**Ответ**: Инструкции для эксперта

```json
{
  "callback": "expert_comment_123_456",
  "status": "success",
...
### 1. **waiting_for_photo**
**Описание**: Ожидание фото для публикации
**Триггер**: После одобрения дайджеста
**Действия**: Прием фото, публикация дайджеста

### 2. **waiting_for_digest_edit**
...
### 3. **expert_commenting_{expert_id}**
**Описание**: Эксперт пишет комментарий
**Триггер**: После выбора новости для комментирования
**Действия**: Прием комментария, сохранение
